...
`sshgate` reads the client identification string, relays the backend server
identification, then reads `SSH_MSG_KEXINIT` and applies the verdict before the
client's KEXINIT reaches the backend. Blocked or pending clients may see the SSH
banner, but they do not complete key exchange with your real `sshd`.
