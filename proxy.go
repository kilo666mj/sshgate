...
func cmdServe(args []string) {
	...
	for _, route := range routes {
		...
		go func() {
			defer wg.Done()
			serveListener(ln, route, store, *allowUnknown, limiter, sem)
		}()
	}
	wg.Wait()
}

func serveListener(ln net.Listener, route Route, store *Store, allowUnknown bool, limiter *rateLimiter, sem *semaphore) {
	for {
		conn, err := ln.Accept()
		if err != nil {
